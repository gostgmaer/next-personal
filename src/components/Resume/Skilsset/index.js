  ...
  return (
    <div className="skill flex flex-col gap-2">
      <div className="name-position flex items-center gap-3">
        <h4 className="name font-semibold text-xl  print:text-xl ">{data.name}</h4>
        <h5 className="level bg-slate-400 text-white font-normal py-0 px-1 rounded">{data.level}</h5>
      </div>
      <div className="keywords">
        <ul className="flex flex-wrap gap-2">
          {data.keywords.map((item) => (
            <li className="bg-blue-500  text-white font-medium print:py-0 py-1 px-1 cursor-pointer rounded transition duration-300 ease-in-out hover:bg-blue-700 hover:text-slate-300" key={item}> {item}</li>
          ))}
        </ul>
      </div>
    </div>
