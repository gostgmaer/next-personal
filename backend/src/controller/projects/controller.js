...
const getData = async (req, res) => {
  try {
    ...
    const objects = await Projects.find(query).sort(filterData.options.sort)
...
const getSingleRecord = async (req, res) => {
  try {
    ...
    const object = await Projects.findById(objectId);
    if (!object) {
      res.status(StatusCodes.NOT_FOUND).json({
        result: object,
...
const remove = async (req, res) => {
  try {
    ...
    const object = await Projects.findOneAndUpdate(
      { _id: ID },
...
const removeMany = async (req, res) => {
  try {
    ...
    const ID = new mongoose.Types.ObjectId(objectId);
    const object = await Projects.bulkWrite(
      { _id: ID },
      { $set: { ...req.body, status: "INACTIVE" } },
      { returnOriginal: false }
    );
